...
export const ResourceCards = (
    ...
    const resNode: models.ResourceStatus[] = props.app && props.app.status.resources.filter(res => res.health?.status  !== models.HealthStatuses.Healthy)
                        ...
                        <div
                            ...
                            <div className='row'>
                                ...
                                {events[res.name] && events[res.name].filter(event => event.type === 'warning' || event.type === 'error').length > 0 ? (
                                    ...
                                    <span className="resource__item-value"> No active events</span>
                                )}
                            </div>
                            <div className='row'>
                                ERROR MESSAGE:
                                {res?.health?.message ? (
                                    <span className='resource__item-value' style={{backgroundColor: '#eff3f5db'}}>
                                        ...
                                        ) : <span className='resource__item-value'>
                                    No error message
                                </span>
                                }
                            </div>
