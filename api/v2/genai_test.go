...
func (m *MockActivity) Store(activity interface{}) error {
...
func TestAnalyzeFailures_Success(t *testing.T) {
	...
	service := NewGenAIService(mockAnalyzer, nil, nil)
...
func TestFeedback_SuccessFeedbackForFailure(t *testing.T) {
	...
	service := NewGenAIService(nil, mockFeedback, nil)
	service.RegisterHandlers(router)
...
func TestFeedback_ShouldNotAcceptV1Body(t *testing.T) {
	// Set Gin to Test Mode
	gin.SetMode(gin.TestMode)
	...
	req, err := http.NewRequest(http.MethodPost, "/feedback", strings.NewReader(
		`{
			"buildUrl": "http://localhost:8000/jenkins/job/ibp-activity-agent/view/change-requests/job/PR-13/30/",
			"comments": "",
			"failureAnalysis": "analysis",
			"failureContext": "context",
			"failureResolution": "resolution",
			"feedbackFor": "failure",
			"pipelineStepsNodeUrl": "http://localhost:8000/jenkins/job/ibp-activity-agent/job/PR-13/30/execution/node/15/log/",
			"positive": true,
			"userId": "admin"
    }`))
	require.NoError(t, err, "Failed to create request")
...
func TestActivity_Success(t *testing.T) {
	...
	// Create an instance of the service with the mocks
	service := NewGenAIService(nil, nil, mockActivity)
	service.RegisterHandlers(router)

	// Define the expected behavior of the mock
	mockActivity.On("Store", mock.Anything).Return(nil)
	...
	req, err := http.NewRequest(http.MethodPost, "/activity", bytes.NewBuffer(requestBody))
	require.NoError(t, err, "Failed to create request")
...
func TestActivity_ShouldBindBodyWithError(t *testing.T) {
	// Set Gin to Test Mode
	gin.SetMode(gin.TestMode)
	...
	// Prepare the request
	req, err := http.NewRequest(http.MethodPost, "/activity", strings.NewReader("not a json body"))
	require.NoError(t, err, "Failed to create request")
...
func TestActivity_StoreError(t *testing.T) {
	// Set Gin to Test Mode
	gin.SetMode(gin.TestMode)
	...
	// Create an instance of the service with the mocks
	service := NewGenAIService(nil, mockActivity, nil)
	service.RegisterHandlers(router)

	// Define the expected behavior of the mock
	mockActivity.On("Store", mock.Anything).Return(errors.New("store error"))
